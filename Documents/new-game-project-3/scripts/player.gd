...
@export var gloante_maxime: int = 600
var gloante_curente: int = gloante_maxime
...
func shoot():
	if gloante_curente <= 0:
		print("Click! Nu mai ai gloanțe.")
		return
		
	gloante_curente -= 1
	print("Poc! Gloanțe rămase: ", gloante_curente)
	...
	var inaltime_pistol = global_position.y + 1.0 
	var plan_tinta = Plane(Vector3.UP, inaltime_pistol)

	var target_3d_pos = plan_tinta.intersects_ray(ray_origin, ray_direction)

	if target_3d_pos != null:
		
		var space_state = get_world_3d().direct_space_state
		var start_pos = global_position + Vector3(0, 1.0, 0)
		
		var shoot_dir = (target_3d_pos - start_pos).normalized()
		var end_pos = start_pos + (shoot_dir * 50.0)
		...
		if result:
			...
			var script_tinta = collider
			
			if collider is Area3D:
				script_tinta = collider.get_parent()

			if script_tinta.has_method("break_bec"):
				script_tinta.break_bec()
				print("Bec spart prin țintire cu mouse-ul!")


func move():
	var input_dir := Input.get_vector("p1left", "p1right", "p1up", "p1down")
